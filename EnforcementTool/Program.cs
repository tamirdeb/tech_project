...
namespace EnforcementTool
{
    ...
    class Program
    {
        ...
        //public static readonly string OverrideHash = "954174ddbda1a563daf22ea79a4395851e159d3f1d88a427874fde1eb0e46ea4"; // password to disable the tool (only on paper)
            ...
            if (File.Exists(stopFile) || File.Exists(stopFileDone))
            {
                try
                {
                    if (File.Exists(stopFile))
                    {
                        ...
                        if (!VerifyHash(content, OverrideHash)) return;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (hwnd != IntPtr.Zero)
                    {
                        ...
                        if (ContainsBannedKeyword(title)) Punish(title, "Window Title");
                        else
                        {
                            string url = GetUrlFromWindow(hwnd);
                            if (!string.IsNullOrEmpty(url) && ContainsBannedKeyword(url)) Punish(url, "URL Scanner");
                        }
                    }
                }
                catch { }
        ...
        public static bool VerifyHash(string input, string hash) { using var sha256 = SHA256.Create(); var bytes = sha256.ComputeHash(Encoding.UTF8.GetBytes(input)); var builder = new StringBuilder(); foreach (var b in bytes) builder.Append(b.ToString("x2")); return builder.ToString().Equals(hash, StringComparison.OrdinalIgnoreCase); }
        ...
        private static bool ContainsBannedKeyword(string t) { t = t.ToLowerInvariant(); foreach (var k in BannedKeywords) if (t.Contains(k)) return true; return false; }
    }
}