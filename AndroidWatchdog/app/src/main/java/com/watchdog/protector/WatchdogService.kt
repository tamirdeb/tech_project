...
class WatchdogService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (packageName == "com.android.settings") {
            // We need to inspect the content to see if "Watchdog Protector" or "Blocker Hero" is on screen.
            // This is resource intensive, so we do it carefully.
            val rootNode = rootInActiveWindow
            if (rootNode != null) {
                val foundWatchdog = findTextInNode(rootNode, "Watchdog Protector")
                val foundBlocker = findTextInNode(rootNode, "Blocker Hero")

                if (foundWatchdog || foundBlocker) {
